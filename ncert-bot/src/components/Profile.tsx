...
const Profile = () => {
  ...
  const [mostUsedBook, setMostUsedBook] = useState<BookUsage | null>(null);
  const [feedback, setFeedback] = useState<Feedback[]>([]);
  const navigate = useNavigate();

  const db = getFirestore(app);

  // Fetch profile, scores, subscription, daily traffic, most used book, and feedback
  useEffect(() => {
  ...
  return (
    <div className="max-w-2xl mx-auto p-4">
      ...
      {/* Logout Button */}
      <button
        onClick={handleLogout} // Triggers handleLogout when clicked
      ...
      <div className="mb-6 p-4 bg-purple-50 rounded-lg">
        ...
        <ul className="mt-2">
          {feedback.map((fb, index) => (
            <li key={index} className="text-gray-700">
              {fb.timestamp.toLocaleDateString()}: {fb.message}
            </li>
          ))}
        </ul>
      </div>
